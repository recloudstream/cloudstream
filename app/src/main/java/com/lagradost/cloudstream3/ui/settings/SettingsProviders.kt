...
import com.lagradost.cloudstream3.utils.SubtitleHelper.getLanguageDataFromCode
            ...
            activity?.getApiProviderLangSettings()?.let { current ->
                val languages = synchronized(APIHolder.apis) {
                    APIHolder.apis.map { it.lang }.toSet()
                        .sortedBy { fromTagToLanguage(it) } + AllLanguagesName
                }
                ...
                val langNames = languages.map {
                    if (it == AllLanguagesName)
                        ...
                        Pair(it, getLanguageDataFromCode(it)?.nameNextToFlagEmoji() ?: "")
                }

                activity?.showMultiDialog(
                    langNames.map { it.second },
                    currentList,
                    getString(R.string.provider_lang_settings),
                    {}) { selectedList ->
                    settingsManager.edit().putStringSet(
                        this.getString(R.string.provider_lang_key),
                        selectedList.map { langNames[it].first }.toMutableSet()
                    ).apply()
