...
class CS3IPlayer : IPlayer {
    ...
    private fun buildMergedDualSubtitleSource(
        ...
        // Skip re-downloading and re-merging if a cached merged file already exists for this pair.
        if (!cacheFile.exists() || cacheFile.length() == 0L) {
            ...
            cacheFile.writeText(DualSubtitleComposer.toWebVtt(mergedSegments))
        }
    ...
    private fun loadSubtitleBytes(
        ...
        // Must run on an IO dispatcher — called from player-load paths that may be on the main thread
        return try {
