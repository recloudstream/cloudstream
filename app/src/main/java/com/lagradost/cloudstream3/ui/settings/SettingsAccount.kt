...
class SettingsAccount : BasePreferenceFragmentCompat(), BiometricCallback {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        getPref(R.string.firebase_sync_key)?.setOnPreferenceClickListener {
            showFirebaseSyncDialog(activity ?: return@setOnPreferenceClickListener false)
            true
        }
    ...
    private fun showFirebaseSyncDialog(activity: FragmentActivity) {
        val binding = com.lagradost.cloudstream3.databinding.DialogFirebaseSyncBinding.inflate(layoutInflater)
        ...
        binding.firebaseProjectIdInput.setText(settings.getString("firebase_project_id", ""))
        binding.firebaseApiKeyInput.setText(settings.getString("firebase_api_key", ""))
        binding.firebaseUrlInput.setText(settings.getString("firebase_url", ""))


        binding.firebaseSaveButton.setOnClickListener {
            val projectId = binding.firebaseProjectIdInput.text.toString().trim()
            val apiKey = binding.firebaseApiKeyInput.text.toString().trim()
            val url = binding.firebaseUrlInput.text.toString().trim()

            if (projectId.isEmpty() || apiKey.isEmpty() || url.isEmpty()) {
                showToast(activity, "All fields are required", android.widget.Toast.LENGTH_SHORT)
                return@setOnClickListener
            }

            if (!projectId.startsWith("1:")) {
                 showToast(activity, "Invalid App ID! It must start with '1:'", android.widget.Toast.LENGTH_LONG)
                 return@setOnClickListener
            }
            ...
            if (com.lagradost.cloudstream3.services.SyncRepoService.initFirebase(activity)) {
                 showToast(activity, getString(R.string.firebase_connected), android.widget.Toast.LENGTH_SHORT)
                 
                 // [FIX] First check if we should restore before scheduling an upload
                 com.lagradost.cloudstream3.services.SyncRepoService.autoSyncCheck(activity)
                 
                 // Trigger an immediate sync after connecting, forcing it to ensure data is uploaded
                 com.lagradost.cloudstream3.services.SyncRepoService.scheduleSync(activity, immediate = true)
                 dialog.dismiss()
            } else {
                 showToast(activity, "Initialization failed. Check logs.", android.widget.Toast.LENGTH_SHORT)
            }
        }
        
        binding.firebaseTestButton.setOnClickListener {
             val projectId = binding.firebaseProjectIdInput.text.toString().trim()
             val apiKey = binding.firebaseApiKeyInput.text.toString().trim()
             val url = binding.firebaseUrlInput.text.toString().trim()

             showToast(activity, "Testing connection...", android.widget.Toast.LENGTH_SHORT)
             com.lagradost.cloudstream3.services.SyncRepoService.testConnection(activity, projectId, apiKey, url) { error ->
                 if (error == null) {
                     showToast(activity, getString(R.string.firebase_connected), android.widget.Toast.LENGTH_LONG)
                 } else {
                     showToast(activity, "Failed: $error", android.widget.Toast.LENGTH_LONG)
                 }
             }
        }

        binding.firebaseUploadButton.isVisible = false
        binding.firebaseDownloadButton.isVisible = false
    }
}
