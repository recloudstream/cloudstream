...
fun SortItem(label: String, sortDescending: Boolean?, onClick: () -> Unit) {
    val arrowIcon = when (sortDescending) {
        true -> painterResource(Res.drawable.arrow_downward)
        false -> painterResource(Res.drawable.arrow_upward)
        null -> null
    }
...
@Composable
fun BaseSortItem(label: String, icon: Painter?, onClick: () -> Unit) {
    BaseSettingsItem(
        label = label,
        widget = {
            if (icon != null) {
...
@Composable
fun CheckboxItem(label: String, pref: PreferenceData<Boolean>) {
