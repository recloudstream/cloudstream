...
/*
 NOTE: This is a simplified and more portable kotlin media3 hls parser. As such codec and mime types
 may be invalid/missing.
 */
...
object HlsPlaylistParser {
    ...
    private
    const val TAG_DATERANGE: String = "#EXT-X-DATERANGE"
    ...
    private
    val REGEX_VIDEO_RANGE: Regex = Regex("VIDEO-RANGE=(SDR|PQ|HLG)")

    private
    val REGEX_CODECS: Regex = Regex("CODECS=\"(.+?)\"")

    private
    val REGEX_SUPPLEMENTAL_CODECS: Regex = Regex("SUPPLEMENTAL-CODECS=\"(.+?)\"")
    ...
    private
    val REGEX_ATTR_DURATION_PREFIXED: Regex = Regex("[:,]DURATION=([\\d\\.]+)\\b")
    ...
    private
    val REGEX_ID: Regex = Regex("[:,]ID=\"(.+?)\"")

    private
    val REGEX_CLASS: Regex = Regex("CLASS=\"(.+?)\"")

    private
    val REGEX_START_DATE: Regex = Regex("START-DATE=\"(.+?)\"")

    private
    val REGEX_CUE: Regex = Regex("CUE=\"(.+?)\"")

    private
    val REGEX_END_DATE: Regex = Regex("END-DATE=\"(.+?)\"")

    private
    val REGEX_PLANNED_DURATION: Regex = Regex("PLANNED-DURATION=([\\d\\.]+)\\b")

    private
    val REGEX_END_ON_NEXT: Regex = compileBooleanAttrPattern("END-ON-NEXT")

    private
    val REGEX_ASSET_URI: Regex = Regex("X-ASSET-URI=\"(.+?)\"")

    private
    val REGEX_ASSET_LIST_URI: Regex = Regex("X-ASSET-LIST=\"(.+?)\"")

    private
    val REGEX_RESUME_OFFSET: Regex = Regex("X-RESUME-OFFSET=(-?[\\d\\.]+)\\b")

    private
    val REGEX_PLAYOUT_LIMIT: Regex = Regex("X-PLAYOUT-LIMIT=([\\d\\.]+)\\b")

    private
    val REGEX_SNAP: Regex = Regex("X-SNAP=\"(.+?)\"")

    private
    val REGEX_RESTRICT: Regex = Regex("X-RESTRICT=\"(.+?)\"")

    private
    val REGEX_VARIABLE_REFERENCE: Regex = Regex("\\{\\$([a-zA-Z0-9\\-_]+)\\}")

    private
    val REGEX_CLIENT_DEFINED_ATTRIBUTE_PREFIX: Regex = Regex("\\b(X-[A-Z0-9-]+)=")
    ...
    object MimeTypes {
        ...
        const val APPLICATION_CEA708: String = "$BASE_TYPE_APPLICATION/cea-608"
                ...
                TYPE_AUDIO -> {
                    ...
                    if (channelsString != null) {
                        val channelCount: Int =
                            Util.splitAtFirst(channelsString, "/").get(0).toInt()
                ...
                TYPE_SUBTITLES -> {
                    ...
                    if (uri != null) {
                        subtitles.add(
                            Rendition(
                                uri,
                                formatBuilder.copy(sampleMimeType = MimeTypes.TEXT_VTT),
                                groupId,
                                name
                            )
                        )
